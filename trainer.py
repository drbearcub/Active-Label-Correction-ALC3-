...
def preprocess_for_qa_json(examples, tokenizer, block_size, current_iteration):
    ...
    for id, courseName, userQuery, completion, groundTruth, pastChat, autoCorrectionIterations, filteredIterations, inference, geo_mean, forced_geo_mean in zip(
            ...
            examples.get("pastchat", [[]] * num_examples),
        ...
        completionEnc = tokenizer(completion_for_training,
            truncation=True,
            max_length=block_size-1,
            add_special_tokens=True)
        ...
        pastChatStr = ""
        if pastChat is not None :
            pastChatStr = pastChatStr + "".join(
...
def main():
    ...
    # -------------------------------------------------------------
    # Add custom special tokens for our prompt markers if not present
    # -------------------------------------------------------------
    special_tokens = {
    ...
    if num_new_tokens > 0:
        logger.info(f"Added {num_new_tokens} special tokens to the tokenizer.")
    ...
    with accelerator.main_process_first():
        qa_datasets = raw_datasets.map(
            ...
            desc="Tokenizing and creating masked labels",
        )

    lm_datasets = qa_datasets
